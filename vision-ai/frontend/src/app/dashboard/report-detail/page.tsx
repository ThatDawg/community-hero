...
export default function ReportDetailPage() {
  ...
  const saveEditing = async () => {
    if (!reportId || !editTitle.trim() || !editDescription.trim() || !report) return;
    setSaving(true);
    try {
  ...
  if (!report) {
    return (
      <div className="p-6 text-center">
        <p className="text-muted-foreground">Report not found or has been deleted.</p>
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex gap-2 items-center">
          ...
          {canEdit && (
            <div className="relative" ref={menuRef}>
              ...
              {menuOpen && (
                <div className="absolute right-0 top-full mt-1 w-44 bg-white dark:bg-zinc-900 border rounded-lg shadow-lg z-50 py-1">
                  <button
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-accent transition"
                  >
                    {copied ? <Copy className="h-4 w-4 text-green-500" /> : <Link className="h-4 w-4" />}
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 w-full px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-950 transition"
                  >
              ...
              {editing ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" onClick={saveEditing} disabled={saving || !editTitle.trim() || !editDescription.trim()}>
                      {saving ? "Saving..." : "Save"}
                    </Button>
            ...
            {report.latitude != null && report.longitude != null && (
              <div className="rounded-lg overflow-hidden border space-y-2">
                ...
                <div className="flex gap-2 px-3 pb-2">
                  <a href={`https://www.google.com/maps?q=${report.latitude},${report.longitude}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">Open in Google Maps</a>
                  <span className="text-xs text-muted-foreground">({report.latitude.toFixed(4)}, {report.longitude.toFixed(4)})</span>
                </div>
              </div>
            )}
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {user && (
            <div className="flex gap-2">
              <Textarea placeholder="Add a comment..." rows={2} value={newComment} onChange={(e) => setNewComment(e.target.value)} />
              <Button onClick={handleComment} disabled={!newComment.trim()}><Send className="h-4 w-4" /></Button>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <ConfirmDialog
        ...
        title="Delete Report"
        message="Are you sure? This action cannot be undone. The report and all its data will be removed."
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="destructive"
